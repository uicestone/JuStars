...
class lev_base {
	...
	public function __construct() {
		...
		self::navtitle();
		//if (!defined('TPLDIR')) define('TPLDIR', 'source/plugin/'.self::$PLNAME.'/template');
	...
	public static function formatatt($str, $source, $num = 4, $isthumb = 0, $style='levformatatt') {
	...
	public static function acscore($spend, $notice = '', $type = 0, $uid = 0) {
		...
		if ($uid && intval($spend) && $type >0 && $type <9) {
			...
			if ($upscore >=0) {
				...
				if (self::isopen('scnotice')) notification_add($uid, 'system', $notice.' &raquo; '.self::$lang['extscore'].' '.$spend);
				return TRUE;
			}
		}
	}
